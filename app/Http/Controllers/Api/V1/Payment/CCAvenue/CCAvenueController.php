...
$order_status = $responseMap['order_status'];
    ...
    if($exploded_reference[2]!='add-money-to-wallet'){
            ...
            $this->makePaymentForRide($exploded_reference[1],$transaction_id);
     ...
     * 
     * */
    public function makePaymentForRide($user_id,$transaction_id){

        $request_detail = User::where('id',$user_id)->first()->requestDetail()->where('is_cancelled', false)->where('user_rated', false)->where('driver_id', '!=', null)->first();
