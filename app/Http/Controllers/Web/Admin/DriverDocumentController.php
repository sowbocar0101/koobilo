...
class DriverDocumentController extends BaseController
{
    ...
    {
        if($driver->owner_id){
        $neededDocument = DriverNeededDocument::whereActive(true)->where('account_type','fleet_driver')->orWhere('account_type', 'both')->get();
        }else{
        $neededDocument = DriverNeededDocument::whereActive(true)->where('account_type','individual')->orWhere('account_type', 'both')->get();
